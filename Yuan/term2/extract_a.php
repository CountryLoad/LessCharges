...
<?php
  ...
  $url5 = 'http://localhost/info_pages/4g_national.html';
  ...
  foreach($e->find('td') as $td){
    if(is_numeric($td->innertext)||(float)($td->innertext)!=0){
      //$pkg_a[$count]->set_cost($td->innertext);
      echo $td->innertext;
  ...
  var_dump( $pkg_a[5]);//->cal(200,500,200).'</br>';
  echo $pkg_a[5]->cal(500,200,100);
